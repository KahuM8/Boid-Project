...
void Scene::loadCore() {
	...
	m_boids.clear();
	for (int i = 0; i < 100; i++) {
		m_boids.push_back(Boid(glm::linearRand(vec3(-1), vec3(1)), glm::sphericalRand(1.0f), false, glm::vec3{ 0,1,0 },false));
	}
...
void Scene::loadCompletion() {
	...
	//yellow boids
	for (int i = 0; i < 75; i++) {
		m_boids.push_back(Boid(glm::linearRand(vec3(-1), vec3(1)), glm::sphericalRand(1.0f), false, glm::vec3{ 1,1, 0 },false));
	}
	
	//draw preditors
	for (int i = 0; i < 2; i++) {
		m_boids.push_back(Boid(glm::linearRand(vec3(-1), vec3(1)), glm::sphericalRand(1.0f), true, glm::vec3{1,0,0},false));
	}
...
void Scene::loadChallenge() {
	...
	m_boids.clear();
	for (int i = 0; i < 3; i++) {
		m_boids.push_back(Boid(glm::linearRand(vec3(-m_bound_hsize), vec3(m_bound_hsize)), glm::sphericalRand(1.0f), false, glm::vec3{ 0.9,0.4, 0.4 }, true));
	}
...
void Scene::draw(const mat4 &proj, const mat4 &view) {
	...
	for (const Boid &b : m_boids) {
		...
		if (b.sphear) {
			model = scale(mat4(1), vec3(3)) * model;
		}
		...
		if (b.preditor == false && b.sphear == false) {
			m_simple_boid_mesh.draw();
		}
		else if(b.preditor == true)
		{
